...
class AccountController extends ApiResponse
{
    ...
    public function index(Request $request)
    {
        ...
        $query->whereRaw("p.nip_nik = '" . $user->data->nip . "'");
        $row = $query->first();
        if ($row) {
            $photo = _diskPathUrl('pegawai', $row->photo, asset('assets/images/nophoto.png'));
            $photo_casual = _diskPathUrl('pegawai', $row->photo_kasual, asset('assets/images/nophoto.png'));
            $gender = "";
        ...
        abort(203, __('response.data_invalid'));
    }
    ...
    public function token(Request $request)
    {
        ...
        $expiry = now()->addWeek();
        $token->update(['expires_at' => $expiry]);
        $access_token =
            [
